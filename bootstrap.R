...
data <- rbind(data, cbind(seq(from = -1, to= 1, length.out = 1000), 
                          dnorm(seq(from = -1, to= 1, length.out = 1000), 
                                mean = 0, sd = sqrt(1/n)), 
                          rep(0, 1000)))
...
ggplot(data, mapping = aes(x = y.bar, y = density, group = B, color = B)) + 
      geom_line() + labs(title = "n = 20") + 
...
data <- rbind(data, cbind(seq(from = -1, to= 1, length.out = 1000), 
                          dnorm(seq(from = -1, to= 1, length.out = 1000), mean = 0, sd = sqrt(1/n)), 
                          rep(0, 1000)))
